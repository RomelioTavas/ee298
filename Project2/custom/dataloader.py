...
class DataLoader():
    ...
    def load_batch(self, batch_size=1):
        path_A = glob('./{}/pia/*.wav'.format(self.dataset_name))
    ...
    def load_batch2(self, batch_size=1):
        path_A = glob('./{}/pia/*.wav'.format(self.dataset_name))
        path_B = glob('./{}/gac/*.wav'.format(self.dataset_name))
        ...
        for i in range(self.n_batches-1):
            ...
            for i, (audio_A_path, audio_B_path) in enumerate(zip(batch_A, batch_B)):
                audio_A, _ = librosa.core.load(audio_A_path, sr=self.config.sampling_rate,
                                               res_type='kaiser_fast')
                audio_B, _ = librosa.core.load(audio_B_path, sr=self.config.sampling_rate,
                                               res_type='kaiser_fast')
            ...
            yield audios_A, audios_B, audio_A_path, audio_B_path
